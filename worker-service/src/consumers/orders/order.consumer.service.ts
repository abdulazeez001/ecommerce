...
import { IRMQMessage } from 'src/common/utils/interfaces';
...
export class OrderWorkerService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly queuePrefix = `${process.env.RABBITMQ_QUEUE_PREFIX}.order.service`;
  ...
  constructor(private readonly orderService: OrderService) {
    ...
    this.channelWrapper.on('connect', () => {
      this.logger.info(
        'Product service consumer channel has connected, listening for messages...',
      );
    });
  ...
  private async handleMessage(channel: ConfirmChannel, msg: ConsumeMessage) {
    ...
    try {
      ...
      switch (eventName) {
        case 'update_order_owner_details':
          this.logger.info(' [Received] %s', eventName);
          await this.orderService.updateOrder(message.data);
  ...
  private async retryMessage(
    ...
    if (retryCount > this.MAX_JOB_RETRIES) {
      ...
      await channel.publish(this.EXCHANGE_NAME, this.ROUTING_KEY, msg.content, {
        deliveryMode: 2,
        mandatory: true,
      });
    } else {
