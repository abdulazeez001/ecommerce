...
export class OrderService {
  constructor(
    ...
    private readonly productsService: OrdersGrpcService,
    private readonly ownersGrpcService: OwnersGrpcService,
  ) {}

  async updateOrder(payload: OwnerServiceMessageInterface) {
    try {
      ...
      await this.productsService.updateOrder(owner_id, update_payload);
      return { done: true };
    } catch (error) {
      throw error;
    }
